%% olfactometer_sensor_measurement_v1
%serves to measure the time needed for the scent in the olfactometer to reach
%the nosepiece

%note that this requires a special set-up with a bottle of EtOH above the
%level of the plastic tube but less than the needle
...
trials = 20; %how many trials to average for finding the travel time? set this
...
for ii = 1:trials 
    writeDigitalPin(ard,etoh_valve1,1); %etoh on
    writeDigitalPin(ard,etoh_valve2,1); %etoh on
    tic
...
%         end
    ...
    writeDigitalPin(ard,etoh_valve1,0); %etoh on
    writeDigitalPin(ard,etoh_valve2,0); %etoh on
    pause(pause_time)
end
% 
...
xlabel('Time [s]')
ylabel('Voltage [V]')
title('EtOH Sensor Response Time')
hold on
