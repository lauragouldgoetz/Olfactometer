...
%thirteenth draft
%added punishment for the false alarm
%LGG 16Aug18
...
filename = strcat('olfactory_',mouse_group);
load(filename)
...
%% pseudorandom scent selection for trials
    ...
    max_trials = 300; %set this high, since want mouse to decide when to stop
...
habituate_time = 0.002; %seconds, set this (not in paper)
habituate_loop = 10; %number of led+water loops
...
if go_scent == 0
    ...
    %     
        ...
        elseif hit_miss_trial == 0
           ...
           %No go trial means no reward
           pause(intertrial_interval) %set the intertrial interval
           disp('no reward or punishment delivered') %testing
        else
           false_alarm_outcome(ii) = 1;
           disp('false alarm')
           %No go trial means punish if lick
           pause(intertrial_interval+punishment_time) %set the intertrial interval
           disp('punishment delivered') %testing
        end
    ...
    %     
        ...
        if start_pos_kill_switch >= 1 && ...
                ...
                sum(miss_outcome(start_pos_kill_switch:ii)) >= lickless_trial_limit_go
   ...
   for ii = 1:max_trials
        ...
        %     
            ...
            if start_pos_kill_switch >= 1 && ...
                    sum(licks_per_trial(start_pos_kill_switch:ii))==0 &&...
                    sum(miss_outcome(start_pos_kill_switch:ii)) >= lickless_trial_limit_go 
                %note that this can be calculated best this way bc misses
                %are only from the go scents and if they are not licking
                %you can use this as the kill switch
                fprintf('Total trials for mouse ID # %d = %d. \n',mouse_id, trials_run)
            else
                continue
            end
...
fprintf('Hit rate = %d. \n', hit_rate)
fprintf('Correct rejection rate = %d. \n', correct_rejection_rate)
